
<img width=100% src="https://capsule-render.vercel.app/api?type=waving&color=FFEB3C&height=80&section=header"/>

[![Typing SVG](https://readme-typing-svg.herokuapp.com/?color=FFEB3C&size=35&center=true&vCenter=true&width=1000&lines=HELLO,+MY+NAME+is+Ryan+Novaes+;I'm+19+years+old;I+am+from+São+Paulo,+SP;Be+Welcome!+:%29)](https://git.io/typing-svg)

<h2></h2>

## About me

<ul>
  <li>Desenvolvedor de Software</li>
  <li>Jovem Aprendiz no @mercado livre</li>
  <li>Segundo semestre da graduação em Análise e Desenvolvimento De Sistemas </li>
  <li>1x Azure</li>
  
</ul>

##  Projects

#### -BitTask
  Desenvolvido utilizando a famosa stack MERN (MongoDB, Express, React, Node.js), juntamente ao Docker. Este projeto oferece um CRUD com sistema de cadastro e login de usuários, permitindo que os usuários visualizem, criem, atualizem e excluam suas respectivas tarefas e projetos.     

#### -PlanNexus(TCC)
  Sistema de gestão de estoque e membros da AAPM(associação de alunos, ex-alunos,pais e mestre). O principal objetivo do projeto é reduzir a morosidade no preenchimento de planilhas e trazer mais clareza sobre os dados de venda e estoque. As stack que utilizei nesse projeto foi React.js, tailwind CSS e Axios.





### ⚪Minhas skils:

[![My Skills](https://skillicons.dev/icons?i=figma,html,css,sass,javascript,typescript,mysql,react,java,angular,nodejs)](https://skillicons.dev)





